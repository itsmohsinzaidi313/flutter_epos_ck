...
class ImportData {
  ...
  Future<bool> import() async {
    ...
    try {
      ...
      if (_validResponse(serverResponse)) {
        ...
        errorMsg = '>>>ERROR ON getCompanyList';
        await _getCompanyList(data[CompanyTable.TABLE_NAME] ?? []);
        log('company Ready.');

        errorMsg = '>>>ERROR ON getOutletList';
        await _getOutletList(data[OutletTable.TABLE_NAME] ?? []);
        log('outlet Ready.');

        errorMsg = '>>>ERROR ON getUsersList';
        await _getUsersList(data['users'] ?? []);
        log('users Ready.');

        errorMsg = '>>>ERROR ON getVatAmountList';
        await _getVatAmountList(data['vatamount'] ?? []);
        log('vatamount Ready.');

        errorMsg = '>>>ERROR ON getTablesList';
        await _getTablesList(data['tables'] ?? []);
        log('tables Ready.');

        errorMsg = '>>>ERROR ON getCategoriesList';
        await _getCategoriesList(data['categories'] ?? []);
        log('categories Ready.');

        errorMsg = '>>>ERROR ON getModifiersList';
        await _getModifiersList(data['modifiers'] ?? []);
        log('modifiers Ready.');

        errorMsg = '>>>ERROR ON getItemMenusList';
        await _getItemList(data['item_menus'] ?? []);
        log('item_menus Ready.');

        errorMsg = '>>>ERROR ON getItemModifiersList';
        await _getItemModifiersList(data['item_modifiers'] ?? []);
        log('item_modifiers Ready.');

        errorMsg = '>>>ERROR ON getCustomersList';
        await _getCustomersList(data['customers'] ?? []);
        log('customers Ready.');

        errorMsg = '>>>ERROR ON getPaymentMethodsList';
        await _getPaymentMethodsList(data['payment_methods'] ?? []);
        log('payment_methods Ready.');

        errorMsg = '>>>ERROR ON getExpenseCategoriesList';
        await _getExpenseCategoriesList(data['expense_categories'] ?? []);
        log('expense_categories Ready.');

        errorMsg = '>>>ERROR ON _getDepartmentList';
        await _getDepartmentList(data['department'] ?? []);
        log('department Ready.');
        ...
        errorMsg = '>>>ERROR ON getDeviceList';
        await _getDeviceList(data['device'] ?? []);
        log('device Ready.');

        errorMsg = '>>>ERROR ON getRegistersList';
      ...
      showMessage('Import Status', 'Total records imported $_rowsAffected');
      await Future.delayed(Duration(seconds: 1));
      return true;
    } catch (e) {
      log(errorMsg, error: e, name: 'Import');
      return false;
    }
  }
  ...
  Future<void> _getCompanyList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(CompanyTable.TABLE_NAME, {
  ...
  Future<void> _getOutletList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(OutletTable.TABLE_NAME, {
      ...
      await _getOutletPrintersList(item['printer_ips'] ?? []);
      showMessage('Importing',
  ...
  Future<void> _getOutletPrintersList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(OutletPrinterTable.TABLE_NAME, {
        OutletPrinterTable.OUTLET_ID: item[OutletPrinterTable.OUTLET_ID],
        OutletPrinterTable.PRINTER_IP: item[OutletPrinterTable.PRINTER_IP],
        OutletPrinterTable.DATE_ADDED: item[OutletPrinterTable.DATE_ADDED],
        OutletPrinterTable.DATE_MODIFIED: item[OutletPrinterTable.DATE_MODIFIED]
      });
      showMessage('Importing',
          'Getting ${OutletPrinterTable.TABLE_NAME} ... $count/${i.length} ');
      count++;
    }
  }
  ...
  Future<void> _getUsersList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(UserTable.TABLE_NAME, {
  ...
  Future<void> _getVatAmountList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(VatAmountTable.TABLE_NAME, {
  ...
  Future<void> _getTablesList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(TablesTable.TABLE_NAME, {
  ...
  Future<void> _getCategoriesList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(CategoryTable.TABLE_NAME, {
  ...
  Future<void> _getModifiersList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(ModifierTable.TABLE_NAME, {
  ...
  Future<void> _getItemList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(ItemTable.TABLE_NAME, {
  ...
  Future<void> _getItemModifiersList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(ItemModifierTable.TABLE_NAME, {
  ...
  Future<void> _getCustomersList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(CustomerTable.TABLE_NAME, {
        CustomerTable.SERVER_ID: item[CustomerTable.SERVER_ID],
        CustomerTable.REMOTE_ID: item[CustomerTable.REMOTE_ID],
  ...
  Future<void> _getPaymentMethodsList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(PaymentMethodTable.TABLE_NAME, {
  ...
  Future<void> _getExpenseCategoriesList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(ExpenseCategoryTable.TABLE_NAME, {
  ...
  Future<void> _getSalesList(List<dynamic> i) async {
    ...
    for (var item in i) {
      final masterId =
          _rowsAffected += await database.insert(SalesMasterTable.TABLE_NAME, {
  ...
  Future<void> getSalesDetailsList(List<dynamic> i, int masterId) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(SalesDetailTable.TABLE_NAME, {
  ...
  Future<void> _getDeviceList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(DeviceTable.TABLE_NAME, {
  ...
  Future<void> _getRegisterList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(RegisterTable.TABLE_NAME, {
        RegisterTable.SERVER_ID: item[RegisterTable.SERVER_ID],
        RegisterTable.REMOTE_ID: item[RegisterTable.REMOTE_ID],
        ...
        RegisterTable.IS_UPLOADED: item[RegisterTable.REGISTER_STATUS],
  ...
  Future<void> _getDepartmentList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      _rowsAffected += await database.insert(DepartmentTable.TABLE_NAME, {
        DepartmentTable.SERVER_ID: item[DepartmentTable.SERVER_ID],
        DepartmentTable.OUTLET_ID: item[DepartmentTable.OUTLET_ID],
        DepartmentTable.COMPANY_ID: item[DepartmentTable.COMPANY_ID],
        DepartmentTable.NAME: item[DepartmentTable.NAME],
        DepartmentTable.DEL_STATUS: item[DepartmentTable.DEL_STATUS],
        DepartmentTable.DATE_ADDED: item[DepartmentTable.DATE_ADDED],
        DepartmentTable.DATE_MODIFIED: item[DepartmentTable.DATE_MODIFIED],
      });
      await _getDepartmentPrintersList(item['printer_ips'] ?? []);
      showMessage('Importing',
          'Getting ${DepartmentTable.TABLE_NAME} ... $count/${i.length} ');
      count++;
    }
  }

  Future<void> _getDepartmentPrintersList(List<dynamic> i) async {
    int count = 1;
    for (var item in i) {
      showMessage('Importing',
          'Getting ${DepartmentPrinterTable.TABLE_NAME} ... $count/${i.length} ');
      count++;
    }
  }
